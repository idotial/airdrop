...
async function prepare() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        // packs.push({ address: rows[i].address, value: rows[i].value.substring(0, rows[i].value.length - 7), id: rows[i].id })
...
function onReceipt(id) {
  return (err, receipt) => {
    ...
    pool.query(`UPDATE ${TB} SET txhash = $1, state = 1 where id = $2`, [receipt, id])
  }
}
    ...
    for (var i = 0; i < 30; i++) {
        if (packs.length > 0) {
            ...
            try {
                ...
                batch.add(web3.eth.sendSignedTransaction.request(txObject.rawTransaction, 'receipt', onReceipt(pack.id)))
            } catch (e) {
...
setInterval(send.bind(this), 3000)
